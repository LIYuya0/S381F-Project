...
const updataDocuments = function(sc, updatedocument, callback) {
...
const findDocuments = function(sc, document, callback) {
	let cursor = db.collection('shopping_cart').find()
	cursor.forEach((doc) => {
	console.log(`Find Document: ${JSON.stringify(document)}`);
	callback();
});
	const client = new MongoClient(url);
...
const handle_Find = function(req, sc){
	...
	client.connect(function(err){
		...
		findDocument(db, sc, function(docs){
			client.close();
...
const handle_delete = function(res, sc) {
	...
	deletedocument['userid'] = sc.userid;
	console.log(deletedocument['_id']);
	console.log(deletedocument['userid']);	
